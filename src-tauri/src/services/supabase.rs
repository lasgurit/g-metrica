// Cliente HTTP para comunicación con Supabase REST API.
// Las credenciales se inyectan en tiempo de compilación desde
// variables de entorno via build.rs — NUNCA hardcodeadas.
use reqwest::header::{HeaderMap, HeaderValue, AUTHORIZATION, CONTENT_TYPE};
// env!() lee constantes inyectadas por build.rs en tiempo de compilación.
// Si la variable no existía al compilar, el build habría fallado antes de
llegar aquí.
...
pub fn new() -> Self {
    ...
    headers.insert("apikey",
        HeaderValue::from_static(SUPABASE_ANON_KEY));
